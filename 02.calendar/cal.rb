...
###################
#カレンダーの雛形
class OneYearCalendar #１年分のカレンダー
  #初期化
  def initialize(date)
    @date = date
    @year = date.year
...
class ThreeMonthsCalendar #３ヶ月分(前月/今月/来月)のカレンダー
  #初期化
  def initialize(date)
    @date = date
    @year = date.year
...
class Options
  ...
  #nameに渡されたオプションが存在するか確認するメソッド
  def has?(name)
    @options.include?(name)
  end

  #nameに渡されたオプションの値を取得するメソッド
  def get(name)
    @options[name]
  end
end
...
class GetDate
  ...
  attr_writer :year, :month, :h #todayは初期化時点の値から変化しないよう制限
  def initialize
    @today = Date.today
    @year = @today.year
    @month = @today.month
...
cal.create_calendar #カレンダーの作成
cal.print_calendar #出力
